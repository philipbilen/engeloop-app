...
export async function searchArtists(query: string): Promise<ArtistProfile[]> {
  ...
  const { data, error } = await supabase
    ...
    .select("artist_profile_id, artist_name, associated_legal_names")
  ...
  // Map the view data back to ArtistProfile type for consistency
  return data.map(item => ({
    id: item.artist_profile_id,
    artist_name: item.artist_name,
    full_legal_name: item.associated_legal_names, // Assuming this maps to full_legal_name
    // Add other fields if necessary, or fetch full profile if needed
    created_at: '', // Placeholder, as view doesn't have it
