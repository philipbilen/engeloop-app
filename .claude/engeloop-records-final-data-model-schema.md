...
category:
  ["data-model", "schema", "infrastructure", "release-management", "metadata"]
tags:
  [
    "database-design",
    "postgresql",
...
- **Formal Separation of Links:** The core principle is enforced by the schema's relations:
  - **Financial Links:** `licensor_shares` (Schedule A) links _only_ to `contact_id`.
  - **Metadata Links:** All credit tables (`release_main_artists`, `track_main_artists`, `release_contributors`, `track_contributors`) link _only_ to `artist_profile_id`.
...
- **Handling Ordered Credits:** The `position` field (e.g., in `release_main_artists`) is essential for managing the _order_ of artists, enabling correct formatting like "Artist 1 & Artist 2".
